...
AddCSLuaFile("menus/pokedex.lua")
AddCSLuaFile("menus/pokemonstats.lua")
AddCSLuaFile("menus/cl_bag.lua")
AddCSLuaFile("menus/cl_pkmnmenu.lua")
AddCSLuaFile("menus/battlemenu.lua")
AddCSLuaFile("menus/newpokedex.lua")
AddCSLuaFile("menus/pcui.lua")
AddCSLuaFile("menus/shopui.lua")
include("shared.lua")
include("items.lua")
...
resource.AddFile("sounds/emerald_006F.wav")
resource.AddFile("sounds/emerald_006e.wav")
resource.AddFile("sounds/Sample 8.wav")
...
function GM:PlayerInitialSpawn(pl)
	if (pkmn_USE_MYSQL) then
		...
		Garrymon.db:Query("SELECT * FROM players WHERE SteamID=\""..steamID.."\"", function(data)
			...
			print( "[Garrymon] Creating new data for "..client:Name() );

			Garrymon.db:Query("INSERT INTO players (SteamID) VALUES (\""..steamID.."\")", function(data)
				print( "[Garrymon] Created data for "..client:Name() );
			end);
		end;
	end);
end;
...
if ( key == KEY_F3 ) then
RunConsoleCommand("pokedex")
end
end )

function GM:ShowHelp() -- This hook is called everytime F1 is pressed.
RunConsoleCommand("togglef1")
end --Ends function
...
pl:SetModel( "models/player/Group03/female_02.mdl" )
